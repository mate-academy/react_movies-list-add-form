...
✓ `NewMovie` should check if `title`, `imgUrl`, `imdbUrl`, `imdbId` are
...
✓ Disable the submit button until all the required fields are filled;
✓ Clear the form after adding a new movie.
✓ Errors should not be shown after clearing the form (change its key to
...
- Replace `<your_account>` with your Github username in the [DEMO LINK](https://Illia-Skladnik.github.io/react_movies-list-add-form/) and add it to the PR description.
