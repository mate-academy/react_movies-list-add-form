...
const page = {
  ...
  getError: (name) => {
    return page.getByDataCy(`movie-${name}`)
      .parents('.field')
      .find('.help.is-danger');
  },
  submitForm: () => {
...
describe('NewMovie', () => {
  ...
  beforeEach(() => {
    ...
    mount(<NewMovie handleAddMovie={onAdd} />);
  });
  ...
  it('should allow to enter a description', () => {
    const description = 'A family of former child heroes, now grown apart, must reunite to continue to protect the world.';
  ...
  it('should allow to enter an imgUrl', () => {
    const imgUrl = 'https://m.media-amazon.com/images/M/MV5BOTdlODM0MTAtMzRiZi00MTQxLWE0MGUtNTNjOGZjNjAzN2E0XkEyXkFqcGdeQXVyMTkxNjUyNQ@@._V1_QL75_UY562_CR35,0,380,562_.jpg';

    page.getByDataCy('movie-imgUrl')
      .type(imgUrl)
      .should('have.value', imgUrl);
  });
  ...
  it('should not clear the form if title is empty', () => {
    ...
    page.getByDataCy('movie-imgUrl')
      .should('have.value', movie.imgUrl);

    page.getByDataCy('movie-imdbUrl')
      .should('have.value', movie.imdbUrl);
